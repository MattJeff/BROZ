...
const VideoCall = () => {
  ...
  const connectionStateRef = useRef('initializing'); // Ref mirror of connectionState for use in closures
  ...
  // Keep connectionStateRef in sync with connectionState for use in async closures
  useEffect(() => {
    ...
    const initializeCall = async () => {
      ...
      socket.on('partner-left', async () => {
        if (isActive) {
          ...
          // Retry match after cooldown: if still searching after 6s, re-emit join-queue
          setTimeout(() => {
            if (isActive && connectionStateRef.current === 'searching') {
        ...
        <>
          ...
          <div className="absolute top-3 right-3 z-20 w-24 h-32 rounded-2xl overflow-hidden border-2 border-white/30 shadow-xl bg-black">
            ...
            <button
              ...
              className="absolute bottom-1.5 right-1.5 w-10 h-10 rounded-full bg-black/80 backdrop-blur-sm flex items-center justify-center text-white hover:bg-black/90 transition-all border border-white/30"
              data-testid="switch-camera-btn-local"
            >
              <svg className="w-5 h-5" viewBox="0 0 1536 1024" fill="none" xmlns="http://www.w3.org/2000/svg">
