...
fn default_db() -> String { "postgres://brozadmin:password@localhost:5432/broz_notification".into() }
...
impl AppConfig {
    pub fn load() -> anyhow::Result<Self> {
        let config = config::Config::builder()
            .add_source(config::Environment::with_prefix("BROZ_NOTIFICATION").separator("__"))
            .build()?;
